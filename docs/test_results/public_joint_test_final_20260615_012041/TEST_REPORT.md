...
- 服务器：`http://1.95.47.47`
- 开始时间：2026-06-15T01:20:42+08:00
- 完成时间：2026-06-15T01:21:42+08:00
- 结果：47/47 通过，100.0%
...
| UPLOAD-A | 唐昊 | 上传公开资料 A | 通过 | HTTP 200，资料 MAT_1781457650_5a9d74b0，余额 100->120 | 100->120 |
| UPLOAD-B | 薛雨凇 | 完全重复文件拦截 | 通过 | HTTP 400: 文件已存在，与资料 MAT_1781457650_5a9d74b0 完全相同（SHA-256 匹配） | 100->100 |
| UPLOAD-H | 李子彤 | 并发上传资料 H | 通过 | HTTP 200，资料 MAT_1781457658_4e1b159b，余额 +20 | 100->120 |
| UPLOAD-F | 王东涵 | 并发上传资料 F | 通过 | HTTP 200，资料 MAT_1781457658_2752ed6c，余额 +20 | 100->120 |
| UPLOAD-D | 薛雨凇 | 并发上传资料 D | 通过 | HTTP 200，资料 MAT_1781457658_bf8f0207，余额 +20 | 100->120 |
| UPLOAD-E | 周子皓 | 并发上传资料 E | 通过 | HTTP 200，资料 MAT_1781457659_0600b92a，余额 +20 | 100->120 |
| UPLOAD-C | 于骐畅 | 并发上传资料 C | 通过 | HTTP 200，资料 MAT_1781457659_816e7137，余额 +20 | 100->120 |
| UPLOAD-G | 谢傲宇 | 并发上传资料 G | 通过 | HTTP 200，资料 MAT_1781457659_ddac6874，余额 +20 | 100->120 |
...
| DL-E-WANG-DENY | 王东涵 | 无 CS301 权限拒绝 | 通过 | HTTP 400: 权限不足: 该资料仅限选修 CS301 的学生下载，批次内余额观测 120->120 | 120->120 |
| DL-F-TANG-DENY | 唐昊 | 无 CS302 权限拒绝 | 通过 | HTTP 400: 权限不足: 该资料仅限选修 CS302 的学生下载，批次内余额观测 120->125 | 120->125 |
| DL-G-LI-DENY | 李子彤 | 白名单外拒绝 | 通过 | HTTP 400: 权限不足: 该资料仅限指定用户下载，批次内余额观测 120->115 | 120->115 |
| DL-F-YU | 于骐畅 | CS302 同课程下载 | 通过 | HTTP 200，42323 bytes，下载哈希一致；批次内余额观测 120->115 | 120->115 |
| DL-E-FANG | 方天宇 | CS301 同课程下载 | 通过 | HTTP 200，126801 bytes，下载哈希一致；批次内余额观测 100->85 | 100->85 |
| DL-G-FANG | 方天宇 | 白名单允许下载 | 通过 | HTTP 200，126327 bytes，下载哈希一致；批次内余额观测 100->85 | 100->85 |
| DL-A-LI | 李子彤 | 公开资料正常下载 | 通过 | HTTP 200，153438 bytes，下载哈希一致；批次内余额观测 120->115 | 120->115 |
| DL-A-FANG | 方天宇 | 公开资料正常下载 | 通过 | HTTP 200，153438 bytes，下载哈希一致；批次内余额观测 90->85 | 90->85 |
| DL-A-XUE | 薛雨凇 | 公开资料正常下载 | 通过 | HTTP 200，153438 bytes，下载哈希一致；批次内余额观测 120->115 | 120->115 |
| DL-BALANCE-NET | 九账号并发批次 | 并发下载通证净变化 | 通过 | {"2023112317": -15, "2023112318": 5, "2023112330": 5, "2023112379": 15, "2023112380": -5, "2023112385": -5, "2023112392": -5, "2023116100": 5} |  |
...
- 本地后端回归测试：`32 passed`。
- 方天宇钱包可读取 5 条链上交易，包含注册铸造、三次资料支付和一次普通转账。
- 管理员删除资料 H 后，删除事件审计接口返回 1 条记录。
- 执行完整 Compose `down/up` 后，区块高度仍为 41、资料数仍为 7、主测试下载审计仍为 6，合约地址未变化。
- 重启后资料 E 的修改名称仍然存在，资料 H 的 `deleted=true` 仍然存在。
- 公网端口 5000、8545 均不可达，仅前端 80 端口对外开放。
- 重启后再次下载资料 A 成功且 SHA-256 一致。该补充操作发生在主测试和截图之后，新增 1 条合法审计记录，因此服务器当前为区块高度 44、下载审计 7 条。
- 最近 10 分钟后端日志未发现 `ERROR`、`Traceback`、RPC 超时或 nonce 冲突。

## 截图证据

- `screenshots/00-test-summary.png`：47/47 通过、九账号最终余额。
- `screenshots/05-admin-global-audit.png`：主测试 6 条下载审计。
- `screenshots/06-fang-wallet-history.png`：方天宇 5 条链上通证流水。
- `screenshots/07-duplicate-upload-rejected.png`：完全重复文件拦截。
- `screenshots/08-course-permission-rejected.png`：课程权限拒绝。
- `screenshots/09-whitelist-permission-rejected.png`：白名单权限拒绝。
- `screenshots/10-original-file-verified.png`：原文件完整可信。
- `screenshots/11-tampered-file-detected.png`：篡改文件被识别。
